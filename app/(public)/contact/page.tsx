...
import { Mail, Phone, MapPin, ChevronDown, CheckCircle2, Loader2, Send, HelpCircle } from "lucide-react";
    ...
    <div className="bg-background vastu-mandala-bg min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-4 font-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20 items-stretch">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between text-left bg-navy text-white rounded-3xl p-8 shadow-premium-lg relative overflow-hidden">
            ...
            <div className="flex flex-col gap-6">
              <h3 className="font-serif text-xl font-bold text-gold-start">Office Headquarters</h3>
              <p className="text-xs text-background-alt/80 font-light leading-relaxed">
                Our main consulting office is based in Bangalore, Karnataka. Visits are strictly by prior appointment only.
              </p>

              <div className="flex flex-col gap-5 mt-4 text-xs font-light">
                <div className="flex items-start gap-3">
                  <MapPin className="w-5 h-5 text-gold-start flex-shrink-0" />
            ...
            {/* Map placeholder */}
            <div className="w-full h-40 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mt-8 text-xs text-background-alt/60 font-light">
              Interactive Map Container (Bangalore, IN)
            </div>
          </div>

          {/* Contact form panel */}
          <div className="lg:col-span-7 bg-white border border-border rounded-3xl p-8 shadow-premium text-left">
            {submitted ? (
              <div className="h-full flex flex-col justify-center items-center text-center gap-4 py-8 animate-fade-in-up">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed font-light max-w-sm">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
                ...
                <div>
                  ...
                  />
                  {errors.subject && (
                    <span className="text-[10px] text-destructive mt-1 block">{errors.subject.message}</span>
                  )}
                </div>
