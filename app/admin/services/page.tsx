...
export default function AdminServicesPage() {
  ...
  const [serviceForm, setServiceForm] = useState({ title: "", category: "residential", price: 150, description: "" });

  const handleOpenAddService = () => {
    setServiceForm({ title: "", category: "residential", price: 150, description: "" });
  ...
  const handleOpenEditService = (id: string) => {
    ...
    if (s) {
      setServiceForm({ title: s.title, category: s.category, price: s.price, description: s.description });
  ...
  const handleSaveService = (e: React.FormEvent) => {
    ...
    if (serviceModal.mode === "add") {
      const newService = {
        ...
        category: serviceForm.category as 'residential' | 'commercial' | 'industrial',
        icon: "Home",
        description: serviceForm.description,
        longDescription: serviceForm.description,
        price: serviceForm.price,
        benefits: ["Improved spatial vibes"],
        process: [{ step: 1, title: "Survey", description: "Reviewing spatial layouts" }],
        offlineAvailable: true,
        onlineAvailable: true,
        image: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=80&w=800"
      };
      setServices([...services, newService]);
    } else if (serviceModal.mode === "edit" && serviceModal.id) {
      setServices(services.map(s => s.id === serviceModal.id ? { ...s, title: serviceForm.title, category: serviceForm.category as 'residential' | 'commercial' | 'industrial', price: serviceForm.price, description: serviceForm.description } : s));
    }
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up text-left">
      <div className="flex justify-between items-center">
        <p className="text-xs text-muted-foreground font-light">Create, edit, and adjust price sheets for consultations.</p>
        <button 
            ...
            <tbody className="divide-y divide-border/60">
              {services.map((item) => (
                <tr key={item.id} className="hover:bg-background-alt/50 transition-colors">
                  ...
                  <td className="p-4 capitalize text-navy-light">{item.category}</td>
        ...
        <div className="fixed inset-0 bg-navy/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-border rounded-2xl p-6 max-w-md w-full shadow-premium-lg text-left animate-fade-in-up">
            <h3 className="font-serif text-lg font-bold text-navy mb-4">
              {serviceModal.mode === "add" ? "Add New Service" : "Edit Vastu Service"}
            </h3>
            ...
            <form onSubmit={handleSaveService} className="flex flex-col gap-4">
              ...
              <div>
                <label className="block text-[10px] font-semibold text-navy uppercase tracking-wider mb-1.5">Description Summary</label>
                <textarea
                  required
                  rows={4}
                  value={serviceForm.description}
                  onChange={(e) => setServiceForm({ ...serviceForm, description: e.target.value })}
                  placeholder="Add details about consultation steps or deliverables..."
